...
def drive_from_force(force):
    ...
    force_angle = wrap_angle(math.atan2(force[1], force[0]) - robot[2])
...
def goal_force():
    ...
    force_to_goal = [mag * strength for mag in (goal[0] - robot[0], goal[1] - robot[1])]
...
def obstacle_force():  
    ...
    for distance in laser_scan.ranges:
        ...
        strength = get_pf_magnitude_exponential(distance)
        if strength > 0:
            ...
            theta = wrap_angle(robot[2] + cur_angle + math.pi)
            vector = [math.cos(theta) * strength, math.sin(theta) * strength]

            #    2. Add this force vector to the 'force_from_obstacles' vector
            force_from_obstacles = add_forces(force_from_obstacles, vector)
...
def get_pf_magnitude_linear(distance):
    ...
    if distance < distance_threshold:
        magnitude = ((distance_threshold - distance) / distance_threshold) * max_strength
    else:
        magnitude = 0
...
# This function returns the magnitude of repulsive force for the input distance
# using an exponential drop-off function
def get_pf_magnitude_exponential(distance):
    ...
    if distance < distance_threshold:
        magnitude = max_strength * math.exp(-distance*RATE)
    else:
        magnitude = 0
