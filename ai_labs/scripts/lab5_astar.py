    ...
    while not fringe.empty():
        ...
        if expand_node.state not in closed:
            ...
            for g in neighbors:
                ...
                node = SearchNode(g, expand_node, expand_node.cost+1)
                priority = node.cost + euclidean_distance_grid(g, goal)  # Compute it's f-value
                fringe.put((priority, node))  # Add it to the fringe
...
def get_neighbors(g, closed):
    ...
    for x,y in itertools.product((g[0]-1, g[0], g[0]+1), (g[1]-1, g[1], g[1]+1)):
