...
def closer_than_epsilon(robot, waypoint, epsilon):
    return math.hypot(robot[0] - waypoint[0], robot[1] - waypoint[1]) < epsilon
...
def nextWaypoint(waypoint):
    ...
    ## Method 1: Epsilon values
    # if closer_than_epsilon(robot, path[path_index], 0.8):
    #     path_index += 1

    ## Method 2: Visible paths
    while path_index < len(path) and discrete_map.clear_path((robot[0], robot[1]), (path[path_index][0], path[path_index][1])):
        path_index +=1

    if path_index >= len(path):
        path_index = len(path)-1

    waypoint.x = path[path_index][0]
    waypoint.y = path[path_index][1]
