...
class Particle:
    ...
    def __repr__(self):
        return "Particle[" + ", ".join(str(v) for v in (self.x, self.y, self.theta, self.w, id(self))) + "]"

    def copy(self):
        return Particle(self.x, self.y, self.theta, self.w)
...
def get_uniform_particle_distribution(numberOfParticles):
    uniformDistribution = []
    for _ in xrange(numberOfParticles):
        uniformDistribution.append(get_random_particle())
...
def initialize_particles(n):
    ...
    particles = get_uniform_particle_distribution(n)
...
def advance_particles():
    ...
    # Create motionmodel params object
    vp = VelocityParams(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, delta_t)
    ...
    rospy.logdebug('particles after advancing\n'+pformat(particles[:10])+'\n')
...
def get_scan_prob(p):
    ...
    # To get the expected distance, call distance_LUT.distance(x,y,angle), where x,y is the location to start from,
    # and angle is the map angle direction to travel.  This will return the distance from the look-up-table (LUT)
    # that we expect an obstacle to be. To use the sensor model, simply call sensor_model(expected_distance,
    # measured_distance, maximum_range) where expected distance is the distance to the expected obstacle,
    # measured_distance is the distance that the laser measured (stored in laser_data.ranges[]), and maximum_range is
    #  the maximum range reading of the laser scanner (stored in laser_data.range_max)

    # This variable will have the angle from the robot that the current scan is pointing To get global angle to pass
    # to LUT from this, do wrap_angle(current_angle + p.theta), where p.theta is the heading of the current particle
    probability = 1
    current_angle = laser_data.angle_min

    for measuredDistance in laser_data.ranges:
        # IN THIS LOOP, PROCESS THE INDIVIDUAL LASER SCANS, USING THE SENSOR MODEL
        expectedDistance = distance_LUT.distance(p.x, p.y, wrap_angle(p.theta + current_angle))
        probability *= sensor_model(expectedDistance, measuredDistance, laser_data.range_max)

        current_angle += laser_data.angle_increment
...
def resample_particles():
    ...
    probabilities = [p.w for p in particles]
    rospy.logdebug("probabilities\n" + pformat(probabilities[:10]))
    sumProbabilities = sum(probabilities)
    for i, _ in enumerate(probabilities):
        probabilities[i] /= sumProbabilities

    newParticleIndices = np.random.choice(range(len(particles)), size=len(particles), replace=True, p=probabilities).tolist()
    newParticles = []
    for i in newParticleIndices:
        newParticles.append(particles[i].copy())
    newParticles.extend(get_uniform_particle_distribution(50))
...
if __name__ == '__main__':
    ...
    rospy.init_node('lab4', anonymous=True, log_level=rospy.INFO)
    ...
    while not rospy.is_shutdown():
        ...
        if got_laser:
            ...
            if (iteration % display_rate) == 0 and len(particles) > 0:
                sname = discrete_map.world_dir + '/' + 'pf_' + discrete_map.world_name + '_' + str(iteration).zfill(
                    4) + '.png'
